...
namespace GUI
{
    public class ExcelExport
    {
        ...
        public const string T_BaoCaoHangHoa= "BaoCaoHangHoa";
        #endregion
        ...
        private MemoryStream GetTemplateStream(string viewName)
        {
         ...
         //   string path = Global.AppPath + Constants.FOLDER_TEMPLATES + Constants.CHAR_FLASH; //+ Constants.FOLDER_EXCELS + Constants.CHAR_FLASH;
            ...
            switch (viewName)
            {
                ...
                case T_BaoCaoHangHoa:
                    // path += Constants.FILE_KEHOACHTD;
                    arrByte = File.ReadAllBytes("BaoCaoHangHoa.xls").ToArray();
                    break;
                #endregion
            }
            // Get stream
        ...
        public bool ExportHH(List<HangHoa> dataSource, ref string fileName, bool isPrintPreview)
        {
            // Check if data is null
        ...
        //    replacer.Add(V_TONGSO, "100");
            ...
            return OutSimpleReport(dataSource, replacer, "BaoCaoHangHoa", isPrintPreview, ref fileName);
        }
